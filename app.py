...
def init_db():
    ...
    try:
        ...
        cur.execute("ALTER TABLE job_applications ADD COLUMN IF NOT EXISTS education_doc TEXT")
        ...
        """)
        # ===== NEW: notifications inbox for employees/team leaders =====
        cur.execute("""
        ...
        """)
        # ===== NEW: task requests (employee proposes -> team leader/admin approves) =====
        cur.execute("""
        ...
        """)
        # ===== NEW: daily attendance check-ins =====
        cur.execute("""
        ...
        """)
        # ===== NEW: call requests (employee asks to call admin/team leader, needs approval) =====
        cur.execute("""
...
def list_promos_with_ids():
    if not DATABASE_URL:
        return []
    try:
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("SELECT id, COALESCE(text_am, text) FROM promos ORDER BY id")
        rows = cur.fetchall()
        cur.close()
        conn.close()
        return [{"id": r[0], "text": r[1]} for r in rows]
    except Exception as e:
        print(f"DB list_promos error: {e}")
        return []
...
def delete_promo(promo_id):
    ...
    try:
        ...
        print(f"DB delete_promo error: {e}")
        return False
...
def add_job_application(job_id, job_title, user_id, name, username, phone, email='', id_number='', selfie_photo=None, education_doc=None):
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute(
        """INSERT INTO job_applications
           (job_id, job_title, user_id, name, username, phone, email, id_number, selfie_photo, education_doc, status)
           VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,'pending') RETURNING id""",
        (job_id, job_title, user_id, name, username, phone, email, id_number, selfie_photo, education_doc)
    )
    new_id = cur.fetchone()[0]
    conn.commit()
    cur.close()
...
def add_notification(employee_id, ntype, message):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO notifications (employee_id, type, message) VALUES (%s,%s,%s)",
        (employee_id, ntype, message)
    )
    conn.commit()
    cur.close()
    conn.close()
    try:
        ...
        print(f"add_notification push error: {e}")

def get_notifications(employee_id, unread_only=False):
    conn = get_db_connection()
    cur = conn.cursor()
    if unread_only:
        cur.execute("SELECT id, type, message, is_read, created_at FROM notifications WHERE employee_id=%s AND is_read=FALSE ORDER BY id DESC", (employee_id,))
    else:
        cur.execute("SELECT id, type, message, is_read, created_at FROM notifications WHERE employee_id=%s ORDER BY id DESC LIMIT 30", (employee_id,))
    rows = cur.fetchall()
    cur.close()
    conn.close()
    return [{"id": r[0], "type": r[1], "message": r[2], "is_read": r[3], "created_at": str(r[4])} for r in rows]
...
# ===== NEW: የስራ ጥያቄ (Task Requests: employee proposes -> team leader/admin approves) =====
def create_task_request(employee_id, title, description=''):
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO task_requests (employee_id, title, description) VALUES (%s,%s,%s) RETURNING id",
...
def get_pending_task_requests():
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("""
        SELECT tr.id, tr.employee_id, e.full_name, tr.title, tr.description, tr.created_at
        FROM task_requests tr JOIN employees e ON e.id = tr.employee_id
        WHERE tr.status='pending' ORDER BY tr.id
    """)
    rows = cur.fetchall()
    cur.close()
    conn.close()
    return [{"id": r[0], "employee_id": r[1], "employee_name": r[2], "title": r[3], "description": r[4], "created_at": str(r[5])} for r in rows]

def get_my_task_requests(employee_id):
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT id, title, description, status, created_at FROM task_requests WHERE employee_id=%s ORDER BY id DESC LIMIT 20", (employee_id,))
    rows = cur.fetchall()
    cur.close()
    conn.close()
    return [{"id": r[0], "title": r[1], "description": r[2], "status": r[3], "created_at": str(r[4])} for r in rows]

def decide_task_request(request_id, approve, decided_by):
    conn = get_db_connection()
    cur = conn.cursor()
    status = 'approved' if approve else 'rejected'
    cur.execute(
        "UPDATE task_requests SET status=%s, decided_by=%s, decided_at=NOW() WHERE id=%s RETURNING employee_id, title",
    ...
    conn.close()
    if row:
        employee_id, title = row
        icon = "✅" if approve else "❌"
        add_notification(employee_id, 'task_decision', f"{icon} ስራዎ '{title}' {'ጸድቋል' if approve else 'ውድቅ ተደርጓል'}።")
        if approve:
            update_employee_field_by_id(employee_id, 'tasks', title, append=True)
    return row

def update_employee_field_by_id(employee_id, field, value, append=False):
    conn = get_db_connection()
    cur = conn.cursor()
    if append:
        cur.execute(f"UPDATE employees SET {field} = {field} || %s WHERE id=%s", (f"\n• {value}", employee_id))
    else:
        cur.execute(f"UPDATE employees SET {field} = %s WHERE id=%s", (value, employee_id))
    conn.commit()
    cur.close()
    conn.close()

# ===== NEW: የመግቢያ ማረጋገጫ (Daily Attendance Check-in) =====
def checkin_attendance(employee_id):
    """Returns True if this is a new check-in today, False if already checked in."""
    conn = get_db_connection()
    cur = conn.cursor()
    try:
        cur.execute("INSERT INTO attendance (employee_id, check_date) VALUES (%s, CURRENT_DATE)", (employee_id,))
        conn.commit()
        result = True
    except Exception:
        conn.rollback()
...
def get_attendance_summary(employee_id, days=30):
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT check_date FROM attendance WHERE employee_id=%s ORDER BY check_date DESC LIMIT %s", (employee_id, days))
    rows = cur.fetchall()
    cur.close()
    conn.close()
    return [str(r[0]) for r in rows]

# ===== NEW: የስልክ/ቪዲዮ ጥሪ ጥያቄ (Call Requests via free Jitsi Meet) =====
def create_call_request(requester_chat_id, requester_name, target_role):
    """target_role: 'admin' or 'team_leader'"""
    room_name = f"Marshalom{requester_chat_id}{int(time.time())}"
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO call_requests (requester_chat_id, requester_name, target_role, room_name) VALUES (%s,%s,%s,%s) RETURNING id",
        (requester_chat_id, requester_name, target_role, room_name)
    )
    new_id = cur.fetchone()[0]
    conn.commit()
    cur.close()
    conn.close()
    return new_id, room_name

def get_pending_call_requests():
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT id, requester_chat_id, requester_name, target_role, room_name FROM call_requests WHERE status='pending' ORDER BY id")
    rows = cur.fetchall()
    cur.close()
    conn.close()
    return [{"id": r[0], "requester_chat_id": r[1], "requester_name": r[2], "target_role": r[3], "room_name": r[4]} for r in rows]

def decide_call_request(request_id, approve):
    conn = get_db_connection()
    cur = conn.cursor()
    status = 'approved' if approve else 'rejected'
    cur.execute(
        "UPDATE call_requests SET status=%s, decided_at=NOW() WHERE id=%s RETURNING requester_chat_id, room_name",
        (status, request_id)
    )
    row = cur.fetchone()
    conn.commit()
...
📞 ለበለጠ መረጃ ይደውሉልን፡ 0931556590</div>
        ...
        <div class="page" id="page-support">
            ...
            <div style="text-align:center;">
                ...
                <div id="branchSelectorBox" style="display:none; margin-bottom:10px; text-align:left;">
                    <div style="font-size:10px; color:#8aa3b5; margin-bottom:4px;">📍 የእርስዎ ቦታ/ቅርንጫፍ ይምረጡ፡</div>
                    <select id="branchSelector" class="input-field" onchange="selectMyBranch()">
        ...
        <div class="page" id="page-admin">
            ...
            <div id="adminContent"><p class="empty-msg">⏳ የአይቲ ሹክሹክታ... / IT Whispers...<br><br>💻 ሰርቨሮቻችን እርስ በእርስ እየተከራከሩ ነው፦ "የ አለቃችን ማርሻሎም ስልክ ነው!" ይላል አንዱ ሰርቨር። "አይደለም፣ የእውነተኛዋ ንግሥት የልዋም ስልክ ነው!" ይላል ሁለተኛው።<br>🤖 ሳተላይቶቻችን የናንተ መሆኑን አረጋግጠው እስኪከፍቱ ድረስ ጥቂት ሰከንዶች ታገሱ...</p></div>
        </div>
    ...
    <button id="bigBackBtn" onclick="showPage('page-home')" style="display:none; position:fixed; top:10px; right:10px; width:50px; height:34px; background:#4a9eff; color:#fff; font-size:10px; font-weight:700; padding:0; border:none; border-radius:10px; box-shadow:0 4px 12px rgba(74,158,255,0.45); z-index:200;">🏠⬅️</button>
...
function renderJobs() {
    ...
    el.innerHTML = jobs.map(j => `
        <div style="background:rgba(255,255,255,0.03); border-radius:10px; padding:8px; margin-bottom:6px; border-left:4px solid #4a9eff;">
            ...
            <div style="font-size:9px; color:#8aa3b5; margin:4px 0;">🎓 የትምህርት ማስረጃ (አማራጭ)</div>
            <input type="file" id="education-${j.id}" accept="image/*,application/pdf" class="input-field" style="padding:6px;">
...
async function applyJob(jobId, jobTitle) {
    ...
    const eduInput = document.getElementById('education-' + jobId);
    let education_doc = null;
    if (eduInput.files && eduInput.files[0]) {
        education_doc = await fileToBase64(eduInput.files[0]);
    }
    await fetch('/api/jobs/apply', {
        method: 'POST', headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({initData, job_id: jobId, job_title: jobTitle, phone, email, id_number, selfie_photo, education_doc})
    });
    alert(t.applySent);
}
...
async function loadAdminPage() {
    ...
    if (!verify.ok) {
        el.innerHTML = '<p class="empty-msg">🚫 ተደራሽነት የለዎትም (የባለቤት አካውንት ብቻ) / Access Denied (Owner Only)<br><br>💻 ሰርቨሮቻችን ምርመራቸውን አጠናቀዋል፦ "ይህማ የአለቃችን የማርሻሎም ስልክ አይደለም! የንግሥት ልዋምም አይደለም!" በማለት በሩን ዘግተውታል።</p>';
        return;
    }
                ...
                ${(d.allEmployees || []).map(e => `
                    <div style="background:rgba(255,255,255,0.03); border-radius:8px; padding:8px; margin-bottom:4px; font-size:10px;">
                        ...
                        <div style="display:flex; gap:4px; margin-top:4px;">
                            <button style="flex:1; font-size:9px; padding:4px; border:none; border-radius:6px; background:#2a5a3a; color:#fff;" onclick="adminCallEmployee('${e.username}')">📞 ደውል</button>
...
async function adminCallEmployee(username) {
    const res = await fetch('/api/admin/call_employee', {
        method: 'POST', headers: {'Content-Type':'application/json','X-Init-Data': initData},
        body: JSON.stringify({username})
    });
    const data = await res.json();
    if (data.ok) {
        alert('✅ የጥሪ ሊንክ ተልኳል! የእርስዎ ሊንክ፡\\n' + data.call_link);
    } else {
...
async function loadBranchSelector() {
    try {
        ...
        const box = document.getElementById('branchSelectorBox');
        const sel = document.getElementById('branchSelector');
        if (branches && branches.length) {
            sel.innerHTML = '<option value="">-- ይምረጡ --</option>' + branches.map(b => `<option value="${b.name}">🏢 ${b.name}</option>`).join('');
            box.style.display = 'block';
        }
    } catch (e) { /* no branches configured yet, keep hidden */ }
}
async function selectMyBranch() {
    const branch = document.getElementById('branchSelector').value;
    if (!branch) return;
    await fetch('/api/customer/set_branch', {
        method: 'POST', headers: {'Content-Type':'application/json'},
        body: JSON.stringify({initData, branch})
    });
}
// ===== INIT =====
loadProducts();
...
def api_jobs_apply():
    ...
    education_doc = body.get('education_doc')  # optional, base64 image or PDF
    name = user.get('first_name', '') if user else body.get('name', 'ስም የለም')
    username = user.get('username', '') if user else ''
    user_id = user.get('id') if user else None
    app_id = add_job_application(job_id, job_title, user_id, name, username, phone, email, id_number, selfie_photo, education_doc)
    ...
    if education_doc:
        caption += "\n🎓 የትምህርት ማስረጃ ተያይዟል (አድሚን dashboard ውስጥ ይታያል)"
...
@app.route('/api/customer/set_branch', methods=['POST'])
def api_customer_set_branch():
    """Lets a customer pick their own location/branch from the Support page (no login needed)."""
    body = request.get_json(silent=True) or {}
    user = verify_telegram_webapp_data(body.get('initData', ''))
    if not user:
    ...
    set_customer_branch(user['id'], branch)
    return jsonify({"ok": True})
...
@app.route('/api/admin/call_employee', methods=['POST'])
def api_admin_call_employee():
    """Admin/super admin directly starts a call with any employee - no request/approval needed."""
    if not require_admin():
        return jsonify({"error": "forbidden"}), 403
    body = request.get_json(silent=True) or {}
    ...
    if not emp.get('telegram_chat_id'):
        return jsonify({"ok": False, "error": "employee has not linked their Telegram yet (ask them to /login once)"}), 400
    room_name = f"Marshalom{emp['telegram_chat_id']}{int(time.time())}"
    call_link = f"https://meet.jit.si/{room_name}"
    requests.post(f"{TELEGRAM_URL}/sendMessage", json={
        'chat_id': emp['telegram_chat_id'],
        'text': f"📞 አድሚን ጥሪ እየጠየቀዎት ነው! ይህን ሊንክ ይክፈቱ፡\n{call_link}"
    })
    return jsonify({"ok": True, "call_link": call_link})
...
🔗: tg://user?id={user_id}"""
        ...
        # 777 easter egg
        if text.strip() == '777':
            requests.post(url, json={'chat_id': chat_id, 'text': '🎉🔓 "777" ደረሰ! የማርሻሎም ምስጢራዊ ኮድ ስራ ላይ ዋለ... ደህና ነው፣ ምንም ልዩ ነገር የለም፣ ግን ጥሩ ስሜት ተሰምቶናል! 😄🚀'})
            return "OK"
...
{emp['tasks'] or 'የለም'}"""
        ...
        # /checkin - daily attendance, free, one tap per day
        if text == '/checkin':
            session = get_session(chat_id)
            if session and session['role'] == 'employee':
                is_new = checkin_attendance(session['employee_id'])
                if is_new:
                    requests.post(url, json={'chat_id': chat_id, 'text': f"✅ መገኘትዎ ተመዝግቧል ለዛሬ ({datetime.now(ETHIOPIA_TZ).strftime('%Y-%m-%d')})።"})
                else:
                    requests.post(url, json={'chat_id': chat_id, 'text': "ℹ️ ዛሬ አስቀድመው check-in አድርገዋል።"})
            else:
                requests.post(url, json={'chat_id': chat_id, 'text': '🔒 መጀመሪያ ይግቡ፡ /login username password'})
            return "OK"

        # /requesttask title | description - employee proposes new work, needs team leader/admin approval
        if text.startswith('/requesttask '):
            session = get_session(chat_id)
            if session and session['role'] == 'employee':
                raw = text[len('/requesttask '):].strip()
                parts = raw.split('|', 1)
                title = parts[0].strip()
                description = parts[1].strip() if len(parts) > 1 else ''
                if title:
                    req_id = create_task_request(session['employee_id'], title, description)
                    requests.post(url, json={'chat_id': chat_id, 'text': f"✅ ስራ ጥያቄ #{req_id} ተልኳል፣ የቲም ሊደር/አድሚን ማጽደቅ በመጠበቅ ላይ።"})
                else:
                    requests.post(url, json={'chat_id': chat_id, 'text': "❌ ይህን ይጠቀሙ፡ /requesttask ርዕስ | ዝርዝር"})
            else:
                requests.post(url, json={'chat_id': chat_id, 'text': '🔒 መጀመሪያ ይግቡ፡ /login username password'})
            return "OK"

        # /mytasks - employee sees their task requests and statuses
        if text == '/mytasks':
            session = get_session(chat_id)
            if session and session['role'] == 'employee':
                my_reqs = get_my_task_requests(session['employee_id'])
                if my_reqs:
                    icon_map = {'pending': '⏳', 'approved': '✅', 'rejected': '❌'}
                    listing = "\n".join([f"{icon_map.get(t['status'],'•')} {t['title']} ({t['status']})" for t in my_reqs])
                else:
                    listing = "ምንም የስራ ጥያቄ የለዎትም። /requesttask ይጠቀሙ።"
                requests.post(url, json={'chat_id': chat_id, 'text': f"📋 የእርስዎ የስራ ጥያቄዎች:\n\n{listing}"})
            else:
                requests.post(url, json={'chat_id': chat_id, 'text': '🔒 መጀመሪያ ይግቡ፡ /login username password'})
            return "OK"

        # /notifications - employee views their unread notifications
        if text == '/notifications':
            session = get_session(chat_id)
            if session and session['role'] == 'employee':
                items = get_notifications(session['employee_id'], unread_only=True)
                if items:
                    listing = "\n\n".join([f"🔔 {n['message']}\n🕐 {n['created_at']}" for n in items])
                    mark_notifications_read(session['employee_id'])
                else:
                    listing = "ምንም አዲስ ማስታወቂያ የለም።"
                requests.post(url, json={'chat_id': chat_id, 'text': listing})
            else:
                requests.post(url, json={'chat_id': chat_id, 'text': '🔒 መጀመሪያ ይግቡ፡ /login username password'})
            return "OK"

        # /calladmin - employee requests an approved video/voice call with the admin (free Jitsi Meet)
        if text == '/calladmin':
        ...
        if is_admin_chat(chat_id):
            ...
            if text.startswith('/setbonus '):
                ...
                if len(parts) == 2:
                    ...
                    emp = get_employee_by_username(parts[0].strip())
                    if emp:
                        add_notification(emp['id'], 'bonus', f"💰 አዲስ ቦነስ ተጨምሮልዎታል፡ {parts[1].strip()}")
                else:
            ...
            if text.startswith('/warn '):
                ...
                if len(parts) == 2:
                    ...
                    emp = get_employee_by_username(parts[0].strip())
                    if emp:
                        add_notification(emp['id'], 'warning', f"⚠️ አዲስ ማስጠንቀቂያ ደርሶዎታል፡ {parts[1].strip()}")
                else:
                    requests.post(url, json={'chat_id': chat_id, 'text': "❌ ይህን ይጠቀሙ፡ /warn username ጽሑፍ"})
                return "OK"

            # /addtask username task_text  (admin assigns a task directly, no approval needed)
            ...
            if text == '/tasksqueue':
                pending = get_pending_task_requests()
                if pending:
                    listing = "\n\n".join([f"#{t['id']} — {t['employee_name']}\n📌 {t['title']}\n📝 {t['description'] or '—'}" for t in pending])
                    listing += "\n\nለማጽደቅ፡ /approvetask <id>\nለመከልከል፡ /rejecttask <id> ምክንያት"
                else:
                    listing = "ምንም ያልታየ ስራ ጥያቄ የለም።"
                requests.post(url, json={'chat_id': chat_id, 'text': f"📋 የስራ ጥያቄዎች ወረፋ:\n\n{listing}"})
                return "OK"

            # /approvetask <id>
            if text.startswith('/approvetask '):
                try:
                    req_id = int(text[len('/approvetask '):].strip())
                    row = decide_task_request(req_id, True, str(chat_id))
                    if row:
                        requests.post(url, json={'chat_id': chat_id, 'text': f"✅ ስራ #{req_id} ጸድቋል።"})
                    else:
                        requests.post(url, json={'chat_id': chat_id, 'text': "❌ ID አልተገኘም።"})
                except ValueError:
                    requests.post(url, json={'chat_id': chat_id, 'text': "❌ ይህን ይጠቀሙ፡ /approvetask 5"})
                return "OK"

            # /rejecttask <id> reason
            if text.startswith('/rejecttask '):
                parts = text[len('/rejecttask '):].split(' ', 1)
                try:
                    req_id = int(parts[0].strip())
                    row = decide_task_request(req_id, False, str(chat_id))
                    if row:
                        requests.post(url, json={'chat_id': chat_id, 'text': f"❌ ስራ #{req_id} ውድቅ ተደርጓል።"})
                    else:
                        requests.post(url, json={'chat_id': chat_id, 'text': "❌ ID አልተገኘም።"})
                except ValueError:
                    requests.post(url, json={'chat_id': chat_id, 'text': "❌ ይህን ይጠቀሙ፡ /rejecttask 5 ምክንያት"})
                return "OK"

            # /callsqueue - admin sees pending call requests
            if text == '/callsqueue':
                pending = get_pending_call_requests()
                if pending:
                    listing = "\n\n".join([f"#{c['id']} — {c['requester_name']} ({c['target_role']} ጠይቋል)" for c in pending])
                    listing += "\n\nለማጽደቅ፡ /approvecall <id>\nለመከልከል፡ /rejectcall <id>"
                else:
                    listing = "ምንም ያልታየ የጥሪ ጥያቄ የለም።"
                requests.post(url, json={'chat_id': chat_id, 'text': f"📞 የጥሪ ጥያቄዎች ወረፋ:\n\n{listing}"})
                return "OK"

            # /approvecall <id> - sends both sides a free Jitsi Meet video call link
            if text.startswith('/approvecall '):
                try:
                    req_id = int(text[len('/approvecall '):].strip())
                    row = decide_call_request(req_id, True)
                    if row:
                        requester_chat_id, room_name = row
                        call_link = f"https://meet.jit.si/{room_name}"
                        requests.post(url, json={'chat_id': requester_chat_id, 'text': f"✅ ጥሪዎ ጸድቋል! ይህን ሊንክ ይክፈቱ፡\n{call_link}"})
                        requests.post(url, json={'chat_id': chat_id, 'text': f"✅ ጸድቋል! ይህንኑ ሊንክ ይክፈቱ፡\n{call_link}"})
                    else:
                        requests.post(url, json={'chat_id': chat_id, 'text': "❌ ID አልተገኘም።"})
                except ValueError:
                    requests.post(url, json={'chat_id': chat_id, 'text': "❌ ይህን ይጠቀሙ፡ /approvecall 5"})
                return "OK"

            # /rejectcall <id>
            if text.startswith('/rejectcall '):
                try:
                    req_id = int(text[len('/rejectcall '):].strip())
                    row = decide_call_request(req_id, False)
                    if row:
                        requester_chat_id, _ = row
                        requests.post(url, json={'chat_id': requester_chat_id, 'text': "❌ የጥሪ ጥያቄዎ ውድቅ ተደርጓል።"})
                        requests.post(url, json={'chat_id': chat_id, 'text': f"❌ ጥሪ #{req_id} ውድቅ ተደርጓል።"})
                    else:
                        requests.post(url, json={'chat_id': chat_id, 'text': "❌ ID አልተገኘም።"})
                except ValueError:
                    requests.post(url, json={'chat_id': chat_id, 'text': "❌ ይህን ይጠቀሙ፡ /rejectcall 5"})
                return "OK"
            ...
            # /listpromos - list all promos with their IDs (so a bad/test one can be deleted)
            if text == '/listpromos':
                items = list_promos_with_ids()
                if items:
                    listing = "\n\n".join([f"#{p['id']}: {p['text'][:120]}" for p in items])
                    listing += "\n\nለማጥፋት፡ /deletepromo <id>"
                else:
                    listing = "ምንም ማስታወቂያ የለም።"
                requests.post(url, json={'chat_id': chat_id, 'text': f"📦 ማስታወቂያዎች:\n\n{listing}"})
                return "OK"

            # /deletepromo <id> - remove a specific promo (e.g. leftover test/bad text)
            if text.startswith('/deletepromo '):
                try:
                    promo_id = int(text[len('/deletepromo '):].strip())
                    if delete_promo(promo_id):
                        requests.post(url, json={'chat_id': chat_id, 'text': f"✅ ማስታወቂያ #{promo_id} ጠፍቷል።"})
                    else:
                        requests.post(url, json={'chat_id': chat_id, 'text': "❌ ID አልተገኘም።"})
                except ValueError:
                    requests.post(url, json={'chat_id': chat_id, 'text': "❌ ይህን ይጠቀሙ፡ /deletepromo 3"})
                return "OK"
        ...
        if text.startswith('/start'):
            ...
            # Normal /start - ONE minimal message, ONE big full-width button. Nothing else.
